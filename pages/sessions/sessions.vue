...
<script>
	export default {
		...
		onLoad: async function(option) {
			const sessionId = option.id.split('-')
			this.to=sessionId[1];
			this.type=sessionId[0];
			const msgs = await getApp().globalData.nim.msgLog.getHistoryMsgs({
				scene: sessionId[0],
				to: sessionId[1],
				limit: 10,
			})
			this.msgList = msgs.map(msg => {
				return {
					id: msg.idClient,
		...
		methods: {
			...
			async test() {
				try {
					...
					const list = await getApp().globalData.nim.session.getSessions({
						"limit": 10,
						"desc": true
					})
